		...
		try {
			...
			if (!maxID.isEmpty() && !minID.isEmpty()) {
				...
				for (int i = 0, len = parallelism; i < len; i++) {
					...
					BasicDBObject idRange = new BasicDBObject("$gte", new ObjectId(
							minBigInteger.add(step.multiply(BigInteger.valueOf(i))).toString(HEXADECIMAL)));
					if (i == len - 1) {
						idRange.append("$lte", new ObjectId(maxBigInteger.toString(HEXADECIMAL)));
					} else {
						idRange.append("$lt", new ObjectId(
								minBigInteger.add(step.multiply(BigInteger.valueOf(i + 1))).toString(HEXADECIMAL)));
					}
