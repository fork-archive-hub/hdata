                ...
                if (!readerConfig.containsKey(JDBCReaderProperties.COLUMNS) && !readerConfig.containsKey(JDBCReaderProperties.EXCLUDE_COLUMNS)) {
                    ...
                    try {
                        ...
                        String selectColumns = "`"
                                + Joiner.on("`, `").join(Utils.getColumns(JDBCUtils.getColumnNames(conn, tableName), excludeColumns)) + "`";
        ...
        if (readerConfig.containsKey(JDBCReaderProperties.SPLIT_BY)) {
            ...
            if (parallelism > 1 || maxFetchSize > 0) {
                ...
                try {
                    ...
                    conn = DataSourceUtils.getDataSource(driver, url, username, password).getConnection();
                    String splitColumn = JDBCUtils.getDigitalPrimaryKey(conn, conn.getCatalog(), null, getFirstTableName(table));
                    if (splitColumn != null) {
