...
public class KafkaConsumer implements Runnable {
	...
	private List<Broker> getBrokers(String zkServers, int sessionTimeout, int connectionTimeout) {
		...
		ZkClient zkClient = new ZkClient(zkServers, sessionTimeout, connectionTimeout, new BytesPushThroughSerializer());
	...
	public void run() {
		...
		if (readerConfig.containsKey(KafkaReaderProperties.START_OFFSET)) {
			List<Broker> brokers = getBrokers(readerConfig.getString(KafkaReaderProperties.ZOOKEEPER_CONNECT), 30000, 30000);
			...
			for (Broker broker : brokers) {
				...
				loop: while (!stop) {
					FetchRequest req = new FetchRequestBuilder().clientId(groupID).addFetch(topic, partition, offset, 1024 * 1024).build();
